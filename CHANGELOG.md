...
## v0.44.1-test — Phase 3.47 Unified Today
...
- 放手成功後加入短暫 settle，再完成資料寫入與畫面清理，令落位感更自然。
...
- 安排 Session：本次時間預設改為所選任務的剩餘時間（不再固定 30 / 120 分）；切換任務時自動刷新帶入該任務的剩餘時間。
...
### Changed
- 目標版面：完全移除佔滿整列的狀態篩選橫框，改為依狀態自動分組——進行中目標直接置頂，暫停與已達成僅在有內容時才顯示分區，封存收入最底的可展開摺疊區。
